...
export function ThemeSwitcher({ fontFamilies }: Props) {
  ...
  return (
    <div className="fixed bottom-5 right-4 z-50">
      {open && (
        <div
          className="mb-3 bg-zinc-900 border border-zinc-700 rounded-2xl shadow-xl w-44 overflow-y-auto"
          style={{ maxHeight: 'calc(100vh - 120px)', padding: '12px' }}
        >

          {/* Cores */}
          <p className="text-zinc-500 text-xs font-semibold mb-1">Cor</p>
          <div className="mb-3">
            {THEMES.map(t => (
              <button
                key={t.id}
          ...
          {/* Estilos */}
          <p className="text-zinc-500 text-xs font-semibold mb-1">Estilo</p>
          <div className="mb-3">
            {STYLES.map(s => (
              <button
                key={s.id}
          ...
          {/* Fontes */}
          <p className="text-zinc-500 text-xs font-semibold mb-1">Fonte</p>
          <div>
            {FONTS.map(f => (
              <button
                key={f.id}
              ...
              >
                <span className="shrink-0" style={{ fontFamily: fontFamilies[f.id] }}>Aa</span>
